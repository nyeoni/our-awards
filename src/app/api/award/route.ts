...
export async function POST(req: Request, res: Response) {
  ...
  const response = await openai.createChatCompletion({
    ...
    messages: [
      {
        role: 'system',
        content: '너는 재미있게 상장의 이름과 내용을 만들어주는 인공지능이야.',
      },
      {
        role: 'user',
        content: `다음은 칭찬의 내용을 포함하는 상장을 만드는 시나리오입니다. 칭찬 내용: "${content}". 상장이름은 8글자 이내로 재미있게 지어주고, 상장내용은 칭찬 내용을 바탕으로 "위 사람은" 으로 시작해서 "무엇으로 어떤 것을 하여" 는 칭찬 내용을 바탕으로 재미있게 간단하게 채워서 넣어주고, 마지막은 "이를 칭찬하여 이 상장을 수여함" 으로 끝나는 형식으로 만들어줘.`,
      },
    ],
    temperature: 0.2,
    max_tokens: 200,
